		...
		return [

			{
				...
				filter: function(source) {
					...
					var tocIds = [];
					var tocElement=$('<p>',{'class':'toc'});
					// heading levels of items in corresponded TOC
					var headingLevels =new Array();
					var top = 0;
            ...
            var elementText=element.text().trim();
						...
						if ((elementText=='[toc]'||elementText=='[TOC]') && element.html()!='<code>[toc]</code>'&&element.html()!='<code>[TOC]</code>'){
							//element = $('<ol>',{'class':'showdown-toc'});
							var tocIdName='tocid_'+top;
							element = $('<p>',{'class':'toc'}).attr('id', tocIdName);//tocElement;//Gary update,the [toc] element will use the output.push(element),then make :tocIds[top++] = output.length,and make the  tocIds[top] insert in output[]current tail
							//element = "";
							headingLevels[top]=new Array();
							headingLevels[top][0]=new Array();
							headingLevels[top][1]=new Array();
							tocIds[top++] = output.length;//gary注：在每个有toc标记的位置，记住当前数组的长度，使toc相应的内容能插在当前数组的最后(toc要插入的位置）。
						}

/* //Gary update,As I think should use [toc] as a line,which make it sample,and have test other markdown tool
						...
						// If so, we'll split the element into two 
							...
							// Otherwise start building a new table of contents.
								...
								//toc = tocElement;
								var tocIdName='tocid_'+top;
							  toc=$('<p>',{'class':'toc'}).attr('id', tocIdName);//tocElement;//Gary update,the [toc] element will use the output.push(element),then make :tocIds[top++] = output.length,and make the  tocIds[top] insert in output[]current tail
						...
						// of contents.
							...
							//for (var j = top - 1; j >= 0; j--) {
								...
								//if (tocIds[j] && !headingLevels[j] && element.prop("tagName")) {
									//if (tocIds[j] && element.prop("tagName")) {
									if (element.prop("tagName")) {
									switch (element.prop("tagName")) {
										...
										case 'H6':
											...
											headingLevels[j][0].push( parseInt(element.prop('tagName').substr(1)));
											aAttr='<a href=#'+element.attr('id')+'>'+element.text()+'</a>'
											headingLevels[j][1].push(aAttr);
											break;
									}
								}
							}
...
//gary addd
          for (var j = 0; j <top; j++)
          {
          	 var tocIdIndex=tocIds[j];
          	 var maxlevel=6;
          	 for(var k=0;k<headingLevels[j][0].length;k++)
          	 {
          	 		     var thisLevel=headingLevels[j][0][k];
          	 ...
          	 for(var k=0;k<headingLevels[j][0].length;k++)
          	 {
          	 	        var liLevelEleStr='';
											...
											for(var level_i=maxlevel;level_i<=thisLevel;level_i++)
											{
                                               liLevelEleStr_start=liLevelEleStr_start+'<span class="toc_left" >';
                                               liLevelEleStr_end=liLevelEleStr_end+'</span>';  
											}
											liLevelEleStr='<span class="toc_item">'+liLevelEleStr_start+headingLevels[j][1][k]+liLevelEleStr_end+'</span>';
											output[tocIdIndex] = $(output[tocIdIndex]).append($(liLevelEleStr));
          	 }
          }
