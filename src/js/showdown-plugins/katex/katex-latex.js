...
* @license MIT
*/
//import asciimathToTex from './asciimath-to-tex';//gary delete
//import katex from './katex';
import katex from './katex.min';
//import showdown from './katex';

import showdown from '../../showdown';
import renderMathInElement from './contrib/auto-render';
...
function renderElements(elements, config, isAsciimath) {
  if (elements.length) {
    for (let i = 0, len = elements.length; i < len; i++) {
      ...
      //const latex = isAsciimath ? asciimathToTex(input) : input;//gary delete
      const latex =input;//gary add
      ...
      filter: (text = '') => {
        ...
        return delimiters.reduce((acc, delimiter) => {
          const test = new RegExp(
            ...
            //`${delimiter.left}(.*?)${delimiter.right}`,//gary test
            'g',
          );
