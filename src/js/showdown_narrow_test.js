...
showdown.subParser('detab', function (text, options, globals) {
  ...
  if(window.tagNarrow)
  {
    text = text.replace(/\t(?=\t)/g, '  '); // g_tab_width  //Gary update
  }
  else
  {
  ...
  text = text.replace(/¨B(.+?)¨A/g, function (wholeMatch, m1) {
    ...
    var numSpaces;       
        if(window.tagNarrow)
        {
           numSpaces = 2 - leadingText.length % 2;  // g_tab_width  //gary upate
        }
        else
        {
  ...
  if(window.tagNarrow)
  {
      text = text.replace(/¨A/g, '  ');  // g_tab_width  //gary update
  }
  else
  {
      text = text.replace(/¨A/g, '    ');  // g_tab_width
